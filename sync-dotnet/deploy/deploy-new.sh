#!/bin/bash
# ─────────────────────────────────────────────────────────────────────────────
# Create new Azure Function App and/or ACA Job for SharePoint C# sync.
...
# Image config (ACA)
ACR_NAME="${ACR_NAME:-}"
ACR_IMAGE_REPO="${ACR_IMAGE_REPO:-sharepoint-sync-dotnet}"
...
command -v dotnet >/dev/null 2>&1 || { echo "[ERROR] dotnet CLI required" >&2; exit 1; }
...
if [[ "$TARGET" = "func" || "$TARGET" = "both" ]]; then
    echo ""
    echo "═══ Creating Function App (.NET isolated) ═══"
    ...
    echo "[INFO] Creating function app (flex consumption, .NET isolated)"
    ...
    az functionapp config appsettings set \
        ...
        --settings "${APP_SETTINGS[@]}" \
            "SYNC_SCHEDULE=$TIMER_SCHEDULE" \
        --output none

    echo "[INFO] Publishing .NET function code"
...
if [[ "$TARGET" = "aca" || "$TARGET" = "both" ]]; then
    ...
    # Step 1: Create job with a placeholder image
    ...
    # Step 3: Update job with the real image
    if [[ -n "$ACR_NAME" ]]; then
